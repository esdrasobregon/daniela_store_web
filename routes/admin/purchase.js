...
router.post('/', async (request, response) => {
    response.set('Cache-control', 'public, max-age =300, s-maxage=600');
    console.log("post purchase method called");
    ...
    try {
        ...
        switch (req) {
            case "addPurchaseList":
                addPurchaseList(request, response);
                break;
            default:
                response.json(result);
                break;
 ...
 */
async function addPurchaseList(request, response) {
    response.set('Cache-control', 'public, max-age =300, s-maxage=600');
    console.log("method add purchase list called");
